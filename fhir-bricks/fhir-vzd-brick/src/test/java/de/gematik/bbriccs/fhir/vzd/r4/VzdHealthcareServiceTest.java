...
class VzdHealthcareServiceTest extends FhirTest {
  ...
  @MethodSource("getHealthcareServiceExamples")
  void shouldSummarizeHealthcareServices(String filename, String content) {
    val resource = this.fhirCodec.decode(VzdHealthcareService.class, content);
  ...
  void shouldDecodeWithoutExplicitTypeHint(String filename, String content) {
    ...
    val vzdResource = assertInstanceOf(VzdHealthcareService.class, resource);
    assertDoesNotThrow(() -> printer.printSummary(vzdResource));
  }

  static Stream<Arguments> getHealthcareServiceExamples() {
    return ResourceLoader.getResourceFilesInDirectory("examples/fhir/valid/vzd/healthcareservice/")
        .stream()
