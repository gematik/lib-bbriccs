...
class PractitionerMutatorProviderTest extends FhirFuzzingMutatorTest {
  ...
  void shouldNotThrowAnything(File f) {
    ...
    val practitioner =
        bundle.getEntry().stream()
            ...
            .findFirst()
            .orElseThrow();
    val mutatorProvider = new PractitionerMutatorProvider();

    mutatorProvider
        .getMutators()
        .forEach(
            m -> {
              assertDoesNotThrow(() -> m.apply(this.ctx, practitioner));
  ...
  @Test
  void shouldFuzzEmptyPractitionerQualificationType() {
