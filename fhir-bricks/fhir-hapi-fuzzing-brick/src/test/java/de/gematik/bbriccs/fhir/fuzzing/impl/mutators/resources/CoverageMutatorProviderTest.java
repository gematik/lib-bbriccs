...
class CoverageMutatorProviderTest extends FhirFuzzingMutatorTest {
  ...
  void shouldNotThrowAnything(File f) {
    ...
    val coverage =
        bundle.getEntry().stream()
            ...
            .map(resource -> (Coverage) resource)
            .findFirst()
            .orElseThrow();
    val mutatorProvider = new CoverageMutatorProvider();

    mutatorProvider
        .getMutators()
        .forEach(
            m -> {
              assertDoesNotThrow(() -> m.apply(this.ctx, coverage));
