...
class ReferenceTypeMutatorProviderTest extends FhirFuzzingMutatorTest {
  ...
  void shouldNotThrowAnything(Reference reference) {
    val mutatorProvider = new ReferenceTypeMutatorProvider();

    mutatorProvider
        .getMutators()
        .forEach(
            m -> {
              assertDoesNotThrow(() -> m.apply(this.ctx, reference));
  ...
  static Stream<Arguments> shouldNotThrowAnything() {
    return Stream.of(new Reference(), new Reference("Hello/World"), new Reference("HelloWorld"))
        .map(Arguments::of);
  }
}
