...
class DeviceMutatorProviderTest extends FhirFuzzingMutatorTest {
  ...
  void shouldNotThrowAnything(File f) {
    ...
    val device =
        bundle.getEntry().stream()
            ...
            .filter(resource -> resource.getResourceType().equals(ResourceType.Device))
            .map(resource -> (Device) resource)
            .findFirst()
            .orElseThrow();
    val mutatorProvider = new DeviceMutatorProvider();

    mutatorProvider
        .getMutators()
        .forEach(
            m -> {
              assertDoesNotThrow(() -> m.apply(this.ctx, device));
