...
class MedicationDispenseMutatorProviderTest extends FhirFuzzingMutatorTest {
  ...
  void shouldNotThrowAnything(File f) {
    ...
    val medicationDispense =
        bundle.getEntry().stream()
            ...
            .map(resource -> (MedicationDispense) resource)
            .findFirst()
            .orElseThrow();
    val mutatorProvider = new MedicationDispenseMutatorProvider();

    mutatorProvider
        .getMutators()
        .forEach(
            m -> {
              assertDoesNotThrow(() -> m.apply(this.ctx, medicationDispense));
  ...
  void shouldFuzzWithStatusReasonReference() {
    ...
    val referenceUrl = "https://gematik.de/fhir/fuzzing/123";
    md.setStatusReason(new Reference(referenceUrl));

    mutatorProvider
        .getMutators()
        .forEach(
            m -> {
              assertDoesNotThrow(() -> m.apply(this.ctx, md));
            });
  }
}
