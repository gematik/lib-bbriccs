...
class MedicationRequestMutatorProviderTest extends FhirFuzzingMutatorTest {
  ...
  void shouldNotThrowAnything(File f) {
    ...
    val medicationRequest =
        bundle.getEntry().stream()
            ...
            .map(resource -> (MedicationRequest) resource)
            .findFirst()
            .orElseThrow();
    val mutatorProvider = new MedicationRequestMutatorProvider();

    mutatorProvider
        .getMutators()
        .forEach(
            m -> {
              assertDoesNotThrow(() -> m.apply(this.ctx, medicationRequest));
