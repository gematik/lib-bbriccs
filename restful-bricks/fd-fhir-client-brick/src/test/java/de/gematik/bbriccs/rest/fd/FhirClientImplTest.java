...
class FhirClientImplTest {
  ...
  void shouldSendRequestViaTestHttpClient(MediaType mediaType) {
    ...
    val fdClient =
        FhirClient.via(httpClient)
            ...
            .usingFhirMimeType(mediaType)
            .withHeaderProvider(
                BasicFhirHeaderProvider.simply(
                    () -> JwtHeaderKey.AUTHORIZATION.createHeader("ABC")))
            .build();

    val response = assertDoesNotThrow(() -> fdClient.request(new TestFhirBRequest()));
  ...
  void shouldServeObservers(MediaType mediaType) {
    ...
    val fdClient =
        FhirClient.via(httpClient)
            ...
            .withHeaderProvider(BasicFhirHeaderProvider.simply(HttpHeader::forCurrentDate))
            .build();
  ...
  private static class TestHttpClient implements HttpBClient {
    ...
    public HttpBResponse send(HttpBRequest bRequest) {
      ...
      return HttpBResponse.status(200).withoutPayload();
    }
  }
