...
To solve this issue, the {brick-fhir-builder} provides a simple concept based on the {link-pattern-builder}. This allows you to build FHIR resources in a structured and type-safe way by applying the same principles we have already used for <<tutorial_match_resources,matching resources>>.
