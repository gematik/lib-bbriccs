...
This is a very common pattern when dealing with FHIR. Imagine a FHIR resource containing a list of extensions. If you want to select a particular one of them, the profile (to be precise the https://hl7.org/fhir/references.html#canonical:[canonical URL] of the profile a resource claims to conform) is your buddy here.
...
Let's have a closer look at the condensed example from the <<tutorial_custom_resources,custom resources tutorial>>.
...
Having such a powerful mechanism makes the <<tutorial_custom_resources,custom resources tutorial>> easier to implement and test.